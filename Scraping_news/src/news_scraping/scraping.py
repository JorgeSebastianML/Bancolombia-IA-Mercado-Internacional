...
# Se crea un diccionario para procesar las fechas
...
class scraping:
    ...
    def scraping_web(self, url, filtrado = True):
        ...
        if filtrado:
            ...
            clean_text = []
            for part in separate_text:
                # Se pasa la parte por el modelo identificador de publicidad
                temp = self.advertising_clean(part)
                if temp != None:
                    clean_text.append(temp)
            text = " ".join(clean_text)
    ...
    def GoogleNews(self, search_list, end_date, duration, n_pages = 1, type_time = "D", lenguage = "en", 
        ...
        # Se calcula el delta de tiempo de la busqueda en funcion al tipo de tiempo que se ingrese
        if type_time == "D":
        ...
        for date in range(len(dates_in_range) - 1):
            ...
            for t in List_Topics: 
                ...
                for i in range(n_pages):
                    ...
                    # Se organizan los resultados
                    Documents = googlenews.page_at(i)
                    ##try:
                    count_1 = 0
                    for doc in Documents:
                        # Se organiza la informacion obtenida en un diccionario
                        filteredDoc = {}
                        filteredDoc["search"] = t
                        # Se utiliza un try para procesar la fecha en caso del que formato cambie, lo que 
                        # ocurre cuando la noticia es muy resente, al menos una semana
                        try:
                            # Se comvierte la el mes de la fecha obtenida utilizando el diccionario de 
                            # conversion
                            Month = convert_dates[doc['date'].split()[0]]
                            # Se crea un string con la fecha convertida
                            fecha = Month + " " + doc['date'].split()[1] + " " + doc['date'].split()[2]
                            # Se convierte el string de la fehca en datetime
                            fecha = datetime.datetime.strptime(fecha, '%b %d, %Y')
                            # Se guarda toda la informacion de la fecha
                            filteredDoc["Year"] = fecha.year
                            filteredDoc["Month"] = fecha.month
                            filteredDoc["Day"] = fecha.day
                            filteredDoc['date'] = fecha.strftime("%Y/%m/%d") 
                        except:
                            # En caso de haber un error en la conversion de la fecha se utiliza la fecha final 
                            # ingresada
                            ...
                            filteredDoc["Day"] = fecha.day
                            filteredDoc['date'] = fecha.strftime("%Y/%m/%d") 
                        # Se verifica que la media no viene vacia para una correctamente asignacion de 
                        # informacion
                        if doc['media'] != None:
                        ...
                        # Se guarda el resto de la informacion en el diccionario creado
                        filteredDoc['title'] = doc['title']
                        filteredDoc['link'] = doc['link']
                        # Se verifica si es un link temporal
                        if 'news.google' in doc['link']:
                            url="https://" + doc['link']
                            ...
                            link = doc["link"]
                        # Se inicializa una bandera que controla el bucle de scraping
                        bandera = True
                        cont = 0
                        # Se crea un bucle para intentar n veces scarpiar una pagina
                        while bandera:
                            # Se verifica el estado de la bandera
                            if bandera:
                                # Se utiliza la funcion scraping_web para retornar informacion de la
                                # pagina de interes
                                #result = self.scraping_web(link)
                                thread = thread_scraping(link)
                                # start the thread
                                thread.start()
                                # wait for the thread to finish
                                thread.join(40)
                                result_2 = thread.results
